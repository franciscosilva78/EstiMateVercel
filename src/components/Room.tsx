        ...
        <div className="flex w-full sm:w-auto gap-3 items-center justify-center sm:justify-end flex-wrap">
          {currentUser.role === "ScrumMaster" && onThemeChange && (
            <div className={`flex gap-1.5 p-1.5 rounded-xl border transition-all duration-1000 ${ui.cardBg} ${ui.cardBorder}`}>
              ...
              <button aria-label="Cyberpunk Theme" onClick={() => onThemeChange("cyberpunk")} className={`w-6 h-6 rounded-full bg-gradient-to-br from-fuchsia-500 to-pink-500 border-2 transition-all ${roomState.theme === "cyberpunk" ? "border-white scale-110 shadow-[0_0_10px_rgba(236,72,153,0.5)]" : "border-transparent opacity-50 hover:opacity-100"}`} title="Cyberpunk"></button>
              <button aria-label="Matrix Theme" onClick={() => onThemeChange("matrix")} className={`w-6 h-6 rounded-full bg-gradient-to-br from-emerald-500 to-teal-500 border-2 transition-all ${roomState.theme === "matrix" ? "border-white scale-110 shadow-[0_0_10px_rgba(16,185,129,0.5)]" : "border-transparent opacity-50 hover:opacity-100"}`} title="Matrix"></button>
              <button aria-label="Ocean Theme" onClick={() => onThemeChange("ocean")} className={`w-6 h-6 rounded-full bg-gradient-to-br from-blue-500 to-cyan-500 border-2 transition-all ${roomState.theme === "ocean" ? "border-white scale-110 shadow-[0_0_10px_rgba(56,187,248,0.5)]" : "border-transparent opacity-50 hover:opacity-100"}`} title="Ocean"></button>
            </div>
          )}
          <button
            onClick={handleShare}
